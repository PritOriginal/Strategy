...
public class Generate : MonoBehaviour
{
    ...
    void forest_generation() 
    {
        for (int x = 0; x < height; x++)
        {
            for (int y = 0; y < width; y++)
            {
                if ((map_matrix[x, y] == (int)Tiles.Land || map_matrix[x, y] == (int)Tiles.Meadows) && UnityEngine.Random.Range(0, 2) == 1)
                {
                    dec_map_matrix[x, y] = (int)DecTiles.Forest;
                }
            }
        }
        drawMap();
        ...
        for (int xc = 0; xc < height; xc++)
        {
            for (int yc = 0; yc < width; yc++)
            {
                ...
                foreach (Vector2Int v in offsets)
                {
                    ...
                    if (x < 0 || x >= height) continue;
                    if (y < 0 || y >= width) continue;
                    if (dec_map_matrix[x, y] == (int)DecTiles.Forest)
                        counter_forest += 1;
                ...
                if (map_matrix[xc, yc] == (int)DecTiles.None)
                {
                    ...
                    {
                        dec_map_matrix[xc, yc] = (int)DecTiles.Forest;
                        continue;
                    }
                }
                if (dec_map_matrix[xc, yc] == (int)DecTiles.Forest)
                {
                    if (counter_land == 3 || counter_land == 4 || counter_land == 5 || counter_land == 6) //
                    {
    ...
    void hills_generation()
    {
        for (int x = 0; x < height; x++)
        {
            for (int y = 0; y < width; y++)
            {
                if ((map_matrix[x, y] == (int)Tiles.Land || map_matrix[x, y] == (int)Tiles.Meadows || map_matrix[x, y] == (int)Tiles.Sand) && dec_map_matrix[x, y] == (int)DecTiles.None && UnityEngine.Random.Range(0, 4) == 1)
                {
...
/*                if (map_matrix[xc, yc] == (int)Tiles.Meadows)
    ...
    void oasis_generation()
    {
        for (int x = 0; x < height; x++)
        {
            for (int y = 0; y < width; y++)
            {
                if ((map_matrix[x, y] == (int)Tiles.Land || map_matrix[x, y] == (int)Tiles.Meadows || map_matrix[x, y] == (int)Tiles.Sand) && dec_map_matrix[x, y] == (int)DecTiles.None && UnityEngine.Random.Range(0, 69) == 1)
                {
